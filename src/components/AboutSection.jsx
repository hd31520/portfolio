...
const AboutSection = ({ id }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 left-0 w-96 h-96 rounded-full bg-green-500/[var(--glow-opacity)] blur-[120px] pointer-events-none"></div>

      <div className="max-w-6xl mx-auto relative z-10">
        <div className="grid md:grid-cols-12 gap-12 items-center">
          {/* Left Column - Image with corrected aspect ratio */}
          <div className="md:col-span-5 relative p-2 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-[24px] shadow-xl group max-w-sm mx-auto w-full">
            <div className="relative rounded-2xl overflow-hidden aspect-square">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[var(--color-background)]/60 via-transparent to-transparent"></div>
            </div>
          </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-4">
              {stats.map((stat, i) => (
                <motion.div 
                  key={i} 
                  whileHover={{ y: -2 }}
                  className="p-5 rounded-[20px] bg-[var(--card-bg)] border border-[var(--card-border)] shadow-md flex flex-col justify-center items-center text-center group hover:border-green-500/20 transition-all duration-300"
                >
