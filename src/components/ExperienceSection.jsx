...
const ExperienceSection = ({ id }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 left-1/4 w-80 h-80 rounded-full bg-green-500/[var(--glow-opacity)] blur-[120px] pointer-events-none"></div>
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        {/* Timeline Container */}
        <div className="relative border-l border-[var(--card-border)] pl-6 sm:pl-8 ml-4 sm:ml-6 space-y-12">
          {experiences.map((exp, index) => (
            ...
            >
              {/* Timeline dot */}
              <div className="absolute -left-[35px] sm:-left-[43px] top-1.5 bg-green-500 rounded-full p-1.5 border-4 border-[var(--color-background)] shadow-md shadow-green-500/20 transition-colors duration-300">
              ...
              <motion.div 
                whileHover={{ y: -3 }}
                className="p-6 rounded-[22px] bg-[var(--card-bg)] border border-[var(--card-border)] shadow-lg flex flex-col gap-3 group hover:border-green-500/20 transition-all duration-300"
              >
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                  <div>
                    <h3 className="text-lg font-bold text-[var(--color-foreground)] group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors">
                      {exp.role}
                    </h3>
                  ...
                  <div className="inline-flex items-center gap-1.5 text-xs text-green-600 dark:text-green-400 bg-green-500/10 border border-green-500/20 px-3 py-1 rounded-full w-fit">
                    <Calendar className="h-3 w-3" />
                    {exp.duration}
                  </div>
                </div>
                <p className="text-[var(--text-muted)] text-xs sm:text-sm leading-relaxed mt-1">
                  {exp.description}
                </p>
              </motion.div>
            </motion.div>
